...
def fetch_ai_response(class_num: int):
    response = ai_api.chat.completions.create(
        ...
        messages=[{
            "role": "user",
            "content": f"Напиши задачи по физике на **{class_num}** классе в формате **JSON**, используя образец: {schema}. 1-ый и 2-ой раунд - по 3 вопроса на каждый, 3 раунд - одна задача по физике. 1-ый раунд: учёные, правила, законы; 2-ой раунд - формулы, величины. Вывод только в JSON. Ищи реальные примеры в интернете. Ответы в запросах должны быть на **разных** местах.**ГЕНЕРИРУЙ СВОИ ВОПРОСЫ. Если вопрос про какого-либо учёного, вопрос должен содержать НЕ имя учёного.**"
        }]
    )
